...
# ---- choose / design your own input here ----------------------------
def my_input(t):
    return 1.0 * (0.7*np.sin(t) + 0.3*np.sin(3*t))   # example
# --------------------------------------------------------------------
...
tri_img   = ax_tri.imshow(np.zeros((model.n_ab, model.n_ab)),
                          extent=[model.u_min, model.u_max,
                                  model.u_min, model.u_max],
                          origin='lower', cmap='RdBu', vmin=-1, vmax=1)

# axis limits
ax_u.set_xlim(0, T_END);  ax_u.set_ylim(np.min(u_t)*1.1, np.max(u_t)*1.1)
ax_u.set_xlabel('time');  ax_u.set_ylabel('u(t)')
ax_f.set_xlim(0, T_END);  ax_f.set_ylim(np.min(f_t)*1.1, np.max(f_t)*1.1)
